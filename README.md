...
本项目默认采用 **交互式问答 + 分模块推进 + 一步步检查** 的学习模式：

- 一次只学一个小主题，不一次性灌输整周内容。
- 每个模块都要等待反馈，再进入下一步。
- 每学完一个模块，都用 2-3 个问题检查理解程度。
- 理解达标后再继续，避免“看过了但其实没掌握”的假完成。
- 质量优先，节奏可以慢，但每一步都要确认理解。

### 协作偏好

- 学习反馈尽量去重、简洁，避免重复段落。
- 每次学习反馈优先给出：答对点、待修正点、结果。
- 学习进度保存时，除了完成情况和理解度，也记录待修正点，便于后续复盘。
- 新增学习资料优先归入对应周的学习记录，再视情况同步到更高层级文件。
...
| [`KNOWLEDGE_BASE/docs-1784018260673/Harness-Engineering-开发设计指导手册.md`](./KNOWLEDGE_BASE/docs-1784018260673/Harness-Engineering-开发设计指导手册.md) | Harness Engineering 学习资料 |
